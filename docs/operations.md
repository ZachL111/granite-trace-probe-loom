...
`granite-trace-probe-loom` is centered on observability work in Scala. The technical target is: Package a Scala local lab for probe analysis with capacity fixtures, allocation and spill reports, and documented operating limits.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 2.
